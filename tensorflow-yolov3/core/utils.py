...
from core import shared_globals
...
def draw_boxes(image, boxes, scores, labels, classes, detection_size,
    ...
    shared_globals.data['A']['Available'] = 0
    for i in range(len(labels)): # for each bounding box, do:
        bbox, score, label = boxes[i], scores[i], classes[labels[i]]
        if (label == "bus" or label == "car" or label == "truck"):
            # print("bbox: {0}, score: {1}, label: {2}".format(bbox, score, label))

            shared_globals.data['A']['Available'] = shared_globals.data['A']['Available'] + 1
